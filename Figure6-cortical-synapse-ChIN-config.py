...
#Load parameters for simulation of subthreshold cortical input to ChIN

with open("config-Figure6-cortical-synapse-ChIN.json",'r') as parameter_file:
...
for cell,cell_info in Cells.items():
    ...
    np.savetxt("Output/Fig-6-Cortical/ChIN/Cortical-synapse-"+parameters_file["single-cell-models"][0].split("/")[-1].split(".")[0]+'-'+brainarea+'-'+synapsenumber+'-'+synapsename+'-Number-of-synapses'+str(Synapse_TOTAL["ChIN_Crtx"])+'-'+name_cells[cell]+'.txt',[tSave,cell_v])
    plt.title("ChIN_LTS")
    plt.legend()
    plt.savefig("Output/Fig-6-Cortical/ChIN/Cortical-synapse-"+parameters_file["single-cell-models"][0].split("/")[-1].split(".")[0]+'-'+'brainarea'+'-'+synapsenumber+'-'+synapsename+'-Number-of-synapses'+str(Synapse_TOTAL["ChIN_Crtx"])+'-'+name_cells[cell]+'.svg')
    plt.clf()
    k=k+1
