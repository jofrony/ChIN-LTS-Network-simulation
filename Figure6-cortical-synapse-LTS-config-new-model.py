...
with open("config-Figure6-cortical-synapse-LTS.json",'r') as parameter_file:
...
for cells in range(1):
    ...
    info_cell.update({"spike_on": recording_netcon})
    
    info_cell.update({"spike_train": spike_time})

    info_cell.update({"soma_voltage":vSave})
    
    Cells.update({"Cell_"+str(cells):info_cell})
...
for cell,cell_info in Cells.items():
    ...
    np.savetxt("Output/Fig-6-Cortical/LTS/Cortical-synapse-"+parameters_file["single-cell-models"][0].split("/")[-1].split(".")[0]+brainarea+'-'+synapsenumber+'-'+synapsename+'-Number-of-synapses'+str(Synapse_TOTAL["LTS_Crtx"])+'-'+name_cells[cell]+'.txt',[tSave,cell_v])
    plt.title("ChIN_LTS")
    plt.legend()
    plt.savefig("Output/Fig-6-Cortical/LTS/Cortical-synapse-"+parameters_file["single-cell-models"][0].split("/")[-1].split(".")[0]+brainarea+'-'+synapsenumber+'-'+synapsename+'-Number-of-synapses'+str(Synapse_TOTAL["LTS_Crtx"])+'-'+name_cells[cell]+'.svg')
    plt.clf()
    k=k+1
