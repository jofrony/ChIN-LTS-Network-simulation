...
with open("config-Figure8-Simulate-Network-timing.json",'r') as parameter_file:
...
for i in range(752):
    np.random.seed(i)
    Th_B = elephant.spike_train_generation.homogeneous_poisson_process(4* pq.Hz,t_stop = parameters_file["simulation_time"] * pq.ms, t_start=0 * pq.ms)
...
for position in position_for_thalamic_input:
    Thalamic_activity =  np.concatenate([np.arange(12500,13200,50),np.arange(15500,16200,50),np.arange(18500,19200,50),np.arange(21500,22200,50),np.arange(24500,25200,50)])
...
for i in range(752):
    np.random.seed(i)
    Crtx_B = elephant.spike_train_generation.homogeneous_poisson_process(2* pq.Hz,t_stop = parameters_file["simulation_time"] * pq.ms, t_start=0 * pq.ms)
...
Crtx_Stim = elephant.spike_train_generation.homogeneous_poisson_process(cortex_frequency* pq.Hz,t_stop = 10000 * pq.ms, t_start=7000 * pq.ms)
...
for cells in range(2):
    ...
    for sec in new_sim.neuron.h.allsec():
        ...
        elif cellssynapseattached[cells] in sec.name() and "Unknown[1]" in sec.name() and "soma" not in sec.name() and "axon" not in sec.name():
...
for cell,cell_info in Cells.items():
    ...
    plt.legend()
    #plt.show() 
    plt.savefig('Output/Fig-8/'+parameters_file["single-cell-models"][0].split("/")[-1].split(".")[0]+'-cortex'+str(cortex_frequency)+'-thalamus-'+str(thalamus_frequency)+'-checker-'+str(checkerweight)+str(Synapse_TOTAL)+str(name_cells[cell])+'.svg')
    np.savetxt('Output/Fig-8/'+parameters_file["single-cell-models"][0].split("/")[-1].split(".")[0]+'-voltage-cortex-'+str(cortex_frequency)+'-thalamus-'+str(thalamus_frequency)+'-checker-'+str(checkerweight)+str(Synapse_TOTAL)+name_cells[cell]+'.txt',[tSave,cell_v])
    plt.clf()
...
plt.legend()
plt.savefig('Output/Fig-8/'+parameters_file["single-cell-models"][0].split("/")[-1].split(".")[0]+'-Checker-LTS-cortex-'+str(cortex_frequency)+'-thalamus-'+str(thalamus_frequency)+'-checker-'+str(checkerweight)+str(Synapse_TOTAL)+'.svg')
plt.clf()

plt.figure(4)
plt.plot(tSave,muscarinic_recording)
plt.title('Muscarinic recording')
plt.savefig('Output/Fig-8/'+parameters_file["single-cell-models"][0].split("/")[-1].split(".")[0]+'-M4-recording-cortex-'+str(cortex_frequency)+'-thalamus-'+str(thalamus_frequency)+'-checker-'+str(checkerweight)+str(Synapse_TOTAL)+'.svg')
plt.clf()

plt.figure(5)
plt.plot(tSave,acetyl_recording)
plt.title('Acetylcholine activation')
plt.savefig('Output/Fig-8/'+parameters_file["single-cell-models"][0].split("/")[-1].split(".")[0]+'-Acetylcholine-cortex-'+str(cortex_frequency)+'-thalamus-'+str(thalamus_frequency)+'-checker-'+str(checkerweight)+str(Synapse_TOTAL)+'.svg')
plt.clf()
